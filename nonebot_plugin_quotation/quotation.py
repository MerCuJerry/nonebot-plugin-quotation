...
async def send_quo(args: str) -> Path:
    ...
    ENSURE_THIS_PATH = (
        (QUOTATION_CONFIG_DIR / str(args + ".json"))
        if not QUO_DIR.is_symlink()
        else (QUOTATION_CONFIG_DIR / str(QUO_DIR.readlink().name + ".json"))
    )
    try:
        ensure_pic_use: list[str] = JSONDecoder().decode(
            ENSURE_THIS_PATH.read_text(encoding="u8")
        )
...
async def save_pic(arg: str, path: Path | None = None, url: str | None = None):
    assert arg != ""
...
async def save_pic_audit(
    arg: str, sender: str, path: Path | None = None, url: str | None = None
):
    assert arg != ""
    assert arg is not None
    ...
    if path is not None:
        ...
        (
            QUOTATION_CACHE_DIR / str(arg + "@" + sender + "@" + md5_hex + ".jpg")
...
async def audit() -> Path | None:
    list_return = list(QUOTATION_CACHE_DIR.iterdir())
    if len(list_return) == 0:
        return None
...
async def return_symlink() -> dict[Path, Path]:
    return {
        dir: dir.readlink() for dir in QUOTATION_DATA_DIR.iterdir() if dir.is_symlink()
    }
